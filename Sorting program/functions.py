    ...
    if folder_name == "archives":
        ...
        for arch_name in files:
            ...
            try:
                ...
                global all_resume
                all_resume += (
                    f"File {all_err_arch} have archive format, but it is not archive.\n"
                )
    if folder_name != "archives":
...
def rename_and_relocation_without_arch(adress: str) -> None:
    ...
    global all_resume
    all_resume += f"This is in the same place: {no_type}\n"
...
def resume_with_arch(*args, adress: str) -> str:
    ...
    for name_fold in args:
        ...
        one_el_line = f"In folder {name_fold} were sorted {count_images} p. Formats: {[el for el in line_of_ras]}\n"
    ...
    for files in archice_files:
        ...
        line.append(named[-1])
    line = set(line)
    arch_part_of_resume = f'I unpacked {count_ar} files. Now you can find it in "archives". They was in this format before: {[el for el in line]}'
